...
of bits, bytes as well as words contained in the strings.  The configuration
...
be escaped using URI encoding (%XX).  Stop words can only be filtered, if 
the B<granularity> is set to I<words>.
