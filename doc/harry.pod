...
This parameter specifies the alphabet size, that is, the number of possible
symbols in the strings.  If the strings consist of bytes, B<alphabet> needs
to be set to 256, whereas if words are analyzed, B<alphabet> has to be
65536.  Note that if the alphabet size is set to some value I<n>, the
symbols of the strings must be between 0 and I<n>-1.  Hence, a normalization
of the symbols is often necessary, for example, the ASCII charcters '0' and
'1' need to be mapped to the bytes 0x00 and 0x01 if I<n> is 2.
