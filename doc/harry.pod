...
of bits, bytes as well as tokens contained in the strings.  The configuration
...
on tokens, that is, each token is considered as a symbol.  If the parameter
B<token_delim> is an empty strings, B<harry> considers each individual
