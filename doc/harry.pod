...
=item B<cache_readers = 8;>

The parameter B<cache_readers> specifies the expected number of readers for
the cache.  The number controls the read-write lock of the cache and
defines how many threads can read from the cache at the exact same time. 
