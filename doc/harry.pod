...
=item B<cache_rwfactor = 8;>

The parameter B<cache_rwfactor> defines the expected ratio between read and
write operations to the cache.  In most cases, the number of read operations
is much higher than the number of write operation. By default the factor is 
set to I<6>. Note that the factor has to be an integer number.  
