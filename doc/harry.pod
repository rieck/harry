...
I<output>.  The similarity measure can be computed based on the granulartiy
of characters or words contained in the strings.  The configuration of this
process, such as the input format, the similarity measure and the output
format, are specified in the file I<config> and can be additionally refined
...
If this parameter is set to I<1>, B<harry> automatically decodes strings
...
the option B<-m> (see B<OPTIONS>).  Parameters of the similarity measures
are organized in individual configuration groups, as for example,
B<dist_levenshtein> for the Levenshtein distance or B<kern_spectrum> for the
spectrum kernel.
...
Levenshtein or edit distance. See configuration group B<dist_levenshtein>.
...
Jaro and Jaro-Winkler distance. See configuration group B<dist_jarowinkler>.
...
Normalized compression distance. See configuration group B<dist_compression>.
...
Kernel-based distance. See configuration group B<dist_kernel>.
...
Spectrum kernel. See configuration group B<kern_spectrum>.

=item I<"kern_wdegree">

Weighted-degree kernel with shifts. See configuration group B<kern_wdegree>.

=item I<"kern_distance">

Distance substitution kernel. See configuration group B<kern_distance>.
...
(also known as Ochiai). See configuration group
B<sim_coefficient>.
