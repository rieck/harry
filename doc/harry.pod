...
automatically splitting the matrix into small submatrices and processing
each of these individually.  This splitting is defined by a string of the
form "I<cols>:I<rows>:I<idx>", where I<cols> and I<rows> define the number
of submatrix columns and rows, respectively, and I<idx> the index of the
submatrix to consider.  For example, the string "I<3:3:7>" corresponds to a
3x3 split of the matrix where the 7th out of the total 9 submatrices is
computed.  For convenience the variable I<idx> ranges from I<1> to
I<rows>*I<cols>.
...
=item B<save_indices = true;>

If this parameter is to I<true> and supported by the output format, the
indices of the strings will be stored in an extra column and row.

=item B<save_labels = false;>

If this parameter is to I<true> and supported by the output format, the
labels of the strings will be stored in an extra column and row.

=item B<save_sources = false;>

If this parameter is to I<true> and supported by the output format, the
sources of the strings will be stored in an extra column and row.

=item B<triangular = false;>

If this parameter is set to I<true> and supported by the output format, a
triangular matrix (upper triangle of the similarity matrix) will be stored. 
The required storage space is reduced by almost a half.
