...
B<harry> [B<options>] [B<-c> I<config>] I<input> [<input>] I<output>
...
strings. It is only considered, if the granularity is set to I<tokens>, 
...
The parameters B<x_range> and B<y_range> are ignore if two input sources are
...
The parameters B<split> is ignore if two input sources are given on the
command line.
...
specifing the dimensions of the matrix, and I<len> is an unsigned 32-bit
integer indicating the length of the following I<array> of similarity values
in single floats (32 bit).  Note that if I<len> != I<xlen> * I<ylen> the
matrix is stored in triangular form, that is, only the upper triangle is
written to standard output. Indices, labels and sources are not
output. This output format is also enables when I<output> is set to 
I<=>, otherwise I<output> is ignored.
