...
some exotic similarity measures.  The focus of B<harry> lies on implicit
...
Levenshtein distance, the Jaro-Winkler distance or the subsequence kernel. 
...
I<output>.  The similarity measure can be computed based on the
granulartiy of characters as well as words contained in the strings.  The
configuration of this process, such as the input format, the similarity
measure and the output format, are specified in the file I<config> and can
be additionally refined using command-line options.
...
configuration can be dumped using the command-line option B<-D> (see
B<OPTIONS>).

=head2 Input formats

B<harry> supports different formats for reading sets of strings,
which may range from plain files to directories and other structured
resources. Following is a list of supported input formats. Additionally, 
the input format can be specified using the command-line option B<-i> 
(see B<OPTIONS>).
...
If this parameter is set to I<1>, B<harry> automatically decodes strings that
contain URI-encoded characters.  That is, substrings of the form %XX are
replaced with the byte represented by the hexadecimal number XX.  
...
If this parameter is set to I<1>, the characters of all input strings will
...
including common distance and kernel functions. The similarity measure
can be specified in the configuration file as well as on the command line
using the option B<-m> (see B<OPTIONS>).
...
Hamming distance. See configuration section B<dist_hamming>.

=item I<"dist_levenshtein">

Levenshtein or edit distance. See configuration section B<dist_levenshtein>.

=item I<"dist_damerau">

Damerau-Levenshtein distance. See configuration section B<dist_damerau>.

=item I<"dist_jaro">

=item I<"dist_jarowinkler">

Jaro and Jaro-Winkler distance. See configuration section B<dist_jarowinkler>.

=item I<"dist_lee">

Lee distance. See configuration section B<dist_lee>

=item I<"dist_compression">

Normalized compression distance. See configuration section B<dist_compression>.

=item I<"dist_bag">

Bag distance. See configuration section B<dist_bag>.

=item I<"dist_kernel">

Kernel-based distance. See configuration section B<dist_kernel>.

=item I<"kern_subsequence">

Subsequence kernel (SSK). See configuration section B<kern_subsequence>.

=item I<"kern_spectrum">

Spectrum kernel. See configuration section B<kern_spectrum>.

=item I<"kern_wdegree">

Weighted-degree kernel with shifts. See configuration section B<kern_wdegree>.

=item I<"kern_distance">

Distance substitution kernel. See configuration section B<kern_distance>.
...
Similarity coefficients: Simpson, Jaccard Braun-Blanquet, Dice (also
known as Soerensen-Dice and Czekanowski), Sokal-Sneath 
(also known as Anderberg), second Kulczynski, Otsuka 
(also known as Ochiai). See configuration section
B<sim_coefficient>.
...
strings.  If this parameter is set the comparison of strings is performed on
words, that is, each word is considered as a symbol.  If the parameter
...
transposition, respectively.  The default costs are I<1.0> for each operation.
...
distance (Winkler, 1990). In contrast to the original formulation, 
a true distance function is implemented, where similar
strings yield a low value and dissimilar strings a high value.
The runtime complexity of a comparison is
...
B<min_sym> is typically set to I<0> and B<max_sym> to I<256>.  For printable
...
This module implements a kernel-based distance. The specified kernel
function is mapped to a Euclidean distance using simple geometry.  The
runtime complexity depends on the kernel function.  The following parameters
are supported:
...
this parameter is specified a squared Euclidean distance is returned which
...
runtime complexity is linear in the length of the strings. The following
parameters are supported:
...
This parameter specifies the length of k-mers (or n-grams) to consider.
...
=item B<save_indices = 1;>

If this parameter is to I<1>, the indices of the strings will be stored in
an extra column and row in some output formats.

=item B<save_labels = 1;>

If this parameter is to I<1>, the labels of the strings will be stored in
an extra column and row in some output formats.

=item B<save_sources = 1;>

If this parameter is to I<1>, the sources of the strings will be stored in
an extra column and row in some output formats.

=item B<compress = 0;>

If this parameter is set to I<1>, the output is stored using zlib compression,
...
=head2 Similarity measure options
...
B<harry> supports only symmetric similiarity measures, that is, m(x,y) =
m(x,y) for all x and y.  This restriction saves considerable run-time and
