...
B<harry> [B<options>] [B<-c> I<config>] I<input> [I<input>] I<output>
...
I<output>.  If one I<input> is given, B<harry> computes the similarities
between all strings in I<input>.  If two I<input> sources are provided,
B<harry> computes only the similarities between the two sources.  The
...
B<lines_regex>).  This input format is also enabled when I<input> is set to
I<->, otherwise I<input> is ignored.
...
where I<len> is a 32-bit unsigned integer in host byte order indicating the
length of the following I<array> containing the string data in bytes. 
Labels cannot be extracted from this representation.  This input format is
also enabled when I<input> is set to I<=>, otherwise I<input> is ignored.
...
the B<granularity> is set to I<tokens>.
...
I<bits>, I<bytes> or I<tokens>. Depending in the granularity a string is 
...
is set to I<tokens>, otherwise it is ignored.
...
of similarity values.  B<col_range> defines a range of indices on the
columns and B<row_range> on the rows of the matrix.  The format of the
ranges is similar to indexing of Python arrays:  A range is given by
"I<start>:I<end>", where I<start> defines the index of the first string and
I<end> defines the index after the last string.  For example, I<"0:4">
...
The parameters B<col_range> and B<row_range> are ignore if two input sources
...
The parameter B<split> is ignore if two input sources are given on the
command line.
...
characters the range can be further narrowed to I<32> and I<126>.  If tokens
are analyzed using the parameter B<token_delim>, B<min_sym> must be set to 0
and B<max_sym> to I<65535>, as the tokens are mapped to integers in this
range.
...
This output format is also enabled when I<output> is set to I<->, otherwise
I<output> is ignored.
...
environments.  The format of the similarity matrix has the following form
  ...
  | xdim (uint32) | ydim (uint32) |
  | array (float) ... |

where I<xdim> and I<ydim> are unsigned 32-bit integers in host byte order
specifing the dimensions of the matrix and I<array> holds the cells of the
matrix as single floats (32 bit).  Indices, labels and sources are not
output.  This output format is also enables when I<output> is set to I<=>,
otherwise I<output> is ignored.
