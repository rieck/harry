...
The parameter B<cache_size> specifies the maximum size of the internal cache in
megabytes (Mb).  The general-purpose cache is used to speed up computations
of B<harry> for several similarity measures.

=item B<global_cache = 0;>

By default B<harry> caches only some internal computations. If this
parameter is set to 1, all similarity values are stored in the cache.  This
feature should only be enabled if many of the compared strings are identical
and thus caching similarity values can provide benefits.
...
This parameter specifies the alphabet size, that is, the maximum number of
unique symbols.  If the strings consist of bytes, B<alphabet> needs to be
set to 256, whereas if words are analyzed using the parameter B<delim>, the
alphabet size has to be 65536.  

Note that if the alphabet size is set to some other value I<n>, the symbols
of the strings must be between 0 and I<n>-1.  Hence, a normalization of the
symbols is often necessary, for example, the ASCII characters '0' and '1'
need to be mapped to the bytes 0x00 and 0x01 if I<n> should be 2.
...
This module implements the compression distance for strings. The compression
is implemented using B<zlib>.  The runtime complexity of a comparison is linear
in the length of the strings, though with a large constant factor.  The
...
The compression distance is only symmetric in theory. If this parameter is
set to 1, the distance is forced to be symmetric by comparing the strings
...
This parameter is a weighting term for gaps.
...
The similarity values are stored as precomputed kernel for libsvm. Note that
the output needs to be postprocessed by replacing the string B<<label>> with
the respective label of each string.
