...
This module implements the weighted degree kernel (Sonnenburg et al., 2007). 
The runtime complexity is linear in the length of the strings.  If the
strings have unequal length, the remaining symbols of the longer string are
ignored, in accordance with the kernel definition.  The following parameters
are supported:
...
k-mers are not extracted but implicitly counted by passing over the strings.

=item B<shift = 0;>

To compensate noise in the strings, the kernel can be computed with "shift". 
The strings are compared multiple times with different positive and negative
offsets up to B<shift> symbols.  The results are average.  The runtime
complexity is increased by twice the value of B<shift>.
...
Sonnenburg, Raetsch, and Rieck. Large scale learning with string kernels. In Large Scale Kernel Machines, pages 73--103. MIT Press, Sept. 2007.

Winkler.  String Comparator Metrics and Enhanced Decision Rules in the Fellegi-Sunter Model of Record Linkage. roceedings of the Section on Survey Research Methods. 354-359, 1990.
